...
public:
    enum AnimationType{ShunZi, LianDui, Plane, JokerBomb, Bomb, Bet,Spring,AntiSpring};
    explicit GamePanel(QWidget *parent = nullptr);
    ~GamePanel();

    QVector<CardPanel*> m_tianLaiZiSet;            //所有的天癞子
    QVector<CardPanel*> m_diLaiZiSet;              //所有的地癞子
    // 更新玩家的分数
    ...
    void initAnimation();
    // 显示游戏动画
    void showAnimation(AnimationType type,Player& player, int bet = 0);
    //展示春天和反春动画
    void beforEndAnimation();
    // 加载角色图像
    QPixmap loadCharacterImage(Character character,Player::Direction direction);
    // 显示分数面板
