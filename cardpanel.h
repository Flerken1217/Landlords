...
public:
    ...
    CardPanel(const CardPanel&);//复制构造函数

    CardPanel& operator=(const CardPanel& other);
    // 设置/获取图片
...
inline uint qHash(CardPanel& cardPanel)
{
    return (cardPanel.getCard().m_point*500+cardPanel.getCard().m_suit);
}

inline bool operator ==(CardPanel& left,CardPanel& right)
{
    return (left.getImage()==right.getImage()
            &&left.getOwner()==right.getOwner()
            &&left.isFrontSide()==right.isFrontSide()
            &&left.isSelected()==right.isSelected()
            &&left.getCard()==right.getCard());
}
