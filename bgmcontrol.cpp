...
void BGMControl::playCardMusic(Cards cards, bool isfirst, MusicType role)
{
    ...
    {
        pt = cards.takeRandomCard().m_point;
    }
...
void BGMControl::OpenVoice()
{
    m_lists[3]->setCurrentIndex(5);
    m_players[3]->play();

    // 创建一个定时器，设置计时时间为3秒
    QTimer *timer = new QTimer(this);
    connect(timer, &QTimer::timeout, this, &BGMControl::stopOpenVoice);
    timer->start(3000); // 开始计时，3000毫秒后触发timeout信号
}

void BGMControl::stopOpenVoice()
{
    // 停止播放器
    m_players[3]->stop();
}
